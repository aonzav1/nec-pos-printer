...
def barcodeln(bar_text) :
    global DEBUG
    list_bits = write_barcode(bar_text,416,135) 
    to_send = list_bits_to_byte(list_bits)    
    # last 2 bytes are length
    # 0x1b, 0x00 = 001b = 27
    # means 416 * 27 = bits
    
    send_test = [0x1b,0x63,0x30,0x02,0x1a,0x2e,0x30,0xa0,0x01,0x87,0x00]
    for b in to_send :
        send_test.append(b)

    written = write_to_adu(dev,send_test)
...
written = write_to_adu(dev,sendBW_1)
if DEBUG : print ("BYTES WROTE " ,written)
written = write_to_adu(dev,sendBW_2)
if DEBUG : print ("BYTES WROTE " ,written)
...
written = write_to_adu(dev,send_LF)
if DEBUG : print ("BYTES WROTE " ,written)

send_CUT = [0x1b,0x63,0x30,0x02,0x1a,0x56,0x41,0xc8];
written = write_to_adu(dev,send_CUT)
if DEBUG : print ("BYTES WROTE " ,written)
